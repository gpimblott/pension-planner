...
export default function InputForm() {
    ...
    return (
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 bg-brand-surface rounded-2xl border border-brand-border p-6 space-y-6 shadow-xl transition-all duration-300 hover:border-brand-primary/10">
                <div className="flex justify-between items-center pb-2 border-b border-brand-border">
                    ...
                    <button
                        type="button"
                        onClick={resetForm}
                        className="text-xs text-brand-muted hover:text-brand-accent font-semibold transition-colors flex items-center gap-1"
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-2 text-sm border border-brand-border bg-brand-bg rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary hover:border-brand-primary/20 transition-all font-medium text-slate-100"
                                min="18"
                                max="100"
                            />
                            <p className="text-[11px] text-brand-muted">{currentAge} years old</p>
                        </div>

                        {/* Retirement Age */}
                        <div className="space-y-1.5">
                            <label htmlFor="retirementAgeInput" className="block text-xs font-bold text-brand-muted uppercase tracking-wider">
                                Retirement Age
                            </label>
                            <input
                                ...
                                max="100"
                            />
                            <p className="text-[11px] text-brand-muted">{retirementAge} years old</p>
                        </div>

                        {/* Current Pot */}
                        <div className="space-y-1.5">
                            <label htmlFor="currentPotInput" className="block text-xs font-bold text-brand-muted uppercase tracking-wider">
                                Current Pension Pot
                            </label>
                            <div className="relative">
                                <span className="absolute left-3 top-2.5 text-brand-muted text-xs font-semibold">£</span>
                                <input
                                    id="currentPotInput"
                                    type="number"
                                    value={currentPot ?? ""}
                                    onChange={handleNumberInput(setCurrentPot)}
                                    className="w-full pl-7 pr-3 py-2 text-sm border border-brand-border bg-brand-bg rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary hover:border-brand-primary/20 transition-all font-medium text-slate-100"
                                    min="0"
                                    step="1000"
                                />
                            </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <p className="text-[11px] text-brand-muted">£{monthlyContribution.toLocaleString()}/mo</p>
                        </div>

                        {/* Employer Contribution */}
                        <div className="space-y-1.5">
                            <label htmlFor="employerContributionInput" className="block text-xs font-bold text-brand-muted uppercase tracking-wider">
                                Employer Contribution
                            </label>
                            <div className="relative">
                                <span className="absolute left-3 top-2.5 text-brand-muted text-xs font-semibold">£</span>
                    ...
                    <div className="pt-4 border-t border-brand-border space-y-3">
                        <div className="flex justify-between items-center">
                            ...
                            <button
                                type="button"
                                onClick={addPension}
                                className="text-xs text-brand-primary hover:opacity-85 font-bold transition-all flex items-center gap-1 hover:underline"
                            >
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {pensions.length > 0 ? (
                            <div className="space-y-3">
                                {/* Header Row */}
                                <div className="grid grid-cols-[1.8fr_1fr_0.9fr_1fr_auto] gap-2 px-1 text-[10px] font-bold text-brand-muted uppercase tracking-wider hidden sm:grid">
                                    <div>Name</div>
                                    <div>Amount (p.a.)</div>
                                    <div>Starts</div>
                                ...
                                {pensions.map((pension, index) => (
                                    <div
                                        key={index}
                                        className="grid grid-cols-1 sm:grid-cols-[1.8fr_1fr_0.9fr_1fr_auto] gap-2 items-center p-3 sm:p-0 bg-brand-bg sm:bg-transparent rounded-lg border border-brand-border sm:border-0"
                                    >
                                        {/* Pension Name */}
                                        <div className="space-y-1 sm:space-y-0">
                                            <span className="text-[10px] font-bold text-brand-muted block sm:hidden">Pension Name</span>
                                            <input
                                                type="text"
                                                value={pension.name}
                                                onChange={(e) => handlePensionChange(index, 'name', e.target.value)}
                                                className="w-full px-2.5 py-1.5 text-xs border border-brand-border rounded-md focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary bg-brand-bg text-slate-100 hover:border-brand-primary/20 transition-colors"
                                        ...
                                        {/* Start Age */}
                                        <div className="space-y-1 sm:space-y-0">
                                            <span className="text-[10px] font-bold text-brand-muted block sm:hidden">Start Age</span>
                                            <input
                                                type="number"
                                        ...
                                        <div className="flex items-center sm:justify-center gap-2 sm:gap-0 pt-1 sm:pt-0">
                                            <span className="text-[10px] font-bold text-brand-muted block sm:hidden">Inflation-linked?</span>
                                            <input
                                                type="checkbox"
                                                checked={pension.isInflationLinked ?? true}
                                                onChange={(e) => handlePensionChange(index, 'isInflationLinked', e.target.checked)}
                                                className="h-3.5 w-3.5 rounded text-brand-primary accent-brand-primary focus:ring-brand-primary/20 border-brand-border bg-brand-bg"
                                            />
                                        </div>
                            ...
                            <div className="text-center py-4 bg-brand-bg border border-dashed border-brand-border rounded-lg text-xs text-brand-muted">
                    ...
                    <div className="pt-4 border-t border-brand-border space-y-3">
                        <div className="flex items-center gap-2">
                            <input
                                id="lumpSumToggle"
                                type="checkbox"
                                className="h-4 w-4 accent-brand-primary rounded cursor-pointer bg-brand-bg border-brand-border"
                                checked={lumpSumEnabled}
                                onChange={(e) => setLumpSumEnabled(e.target.checked)}
                            />
                            <label htmlFor="lumpSumToggle" className="text-xs sm:text-sm font-bold text-slate-100 flex items-center gap-1.5 cursor-pointer select-none hover:text-brand-primary transition-colors">
                                <svg className="w-4 h-4 text-brand-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${
                            lumpSumEnabled ? 'max-h-96 opacity-100 bg-brand-bg p-4 rounded-xl border border-brand-border space-y-4' : 'max-h-0 opacity-0'
                        }`}>
                            <div className="space-y-2">
                                <span className="block text-2xs font-bold text-brand-muted uppercase tracking-wider">Lump Sum Calculation Type</span>
                                <div className="flex gap-4">
                                    <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-300 hover:text-brand-primary transition-colors cursor-pointer">
                                        <input
                                            type="radio"
                                            name="lumpSumType"
                                            value="percentage"
                                            ...
                                            className="accent-brand-primary h-3.5 w-3.5 bg-brand-bg border-brand-border"
                                        />
                                        Percentage of Pot
                                    </label>
                                    <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-300 hover:text-brand-primary transition-colors cursor-pointer">
                            ...
                            {lumpSumType === 'percentage' ? (
                                <div className="space-y-1.5">
                                    <div className="flex justify-between items-center text-xs">
                                        <label htmlFor="lumpSumPctSlider" className="font-semibold text-slate-300">
                                            Percentage: {lumpSumPct}%
                                        </label>
                                        <span className="text-[10px] text-brand-muted font-semibold">(25% is standard tax-free cash limit)</span>
                                    </div>
                                    <input
                                        id="lumpSumPctSlider"
                                        type="range"
                                ...
                                <div className="space-y-1.5">
                                    <label htmlFor="lumpSumAmountInput" className="block text-xs font-semibold text-slate-300">
                                        Cash Amount (£)
                                    </label>
                                    <div className="relative">
                                        <span className="absolute left-3 top-2 text-brand-muted text-xs font-medium">£</span>
                                        <input
                                            id="lumpSumAmountInput"
                                            type="number"
                                            value={lumpSumAmount ?? ""}
                                            onChange={handleNumberInput(setLumpSumAmount)}
                                            className="w-full pl-7 pr-3 py-1.5 text-xs border border-brand-border bg-brand-surface rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary font-medium text-slate-100"
                                            min="0"
                                            step="5000"
                                        />
                                    </div>
                                    <p className="text-[10px] text-brand-muted">Specify the nominal cash sum to take at retirement.</p>
                                </div>
                            )}
                        </div>
                    </div>
